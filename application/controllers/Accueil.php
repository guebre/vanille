...
class Accueil extends CI_Controller{
    ...
    public function  index(){ 
      ...
      //Allow admin to login the systeme
   /* public function login(){

        $error_data = array('login_fail'=>null);
        $data_usr= array();
        $this->form_validation->set_rules('usrname','Email','trim|required|valid_email');
        $this->form_validation->set_rules('usrpass','Password','required');
        ...
        if($this->form_validation->run() == FALSE){
            ...
            $this->load->view('template/header');
            $this->load->view('adminsyst/signin',$error_data);
            $this->load->view('template/footer');

        }
        else{ // Les Données ont été saisies dans le formulaire
            //Passons à la vérification des infos envoyées
            
            //Récuperation des données
            $data_usr["usrname"]=$this->input->post('usrname');
            $data_usr["usrpass"]=$this->input->post('usrpass');
            
            //Si l'utilisateur existe dans le système
            if($this->admin_model->get($data_usr) == 1){

                // User existe dans le systeme
                // Si on a un seul enregistrement 
                // Initialisation des variables de session
                $session_user = array(
                   'pseudo'=> $data_usr['usrname']
                );
                $this->session->set_userdata($session_user);
                $this->admin_model->loginAt($data_usr['usrname']);
                //Redirection dans le panneau d'administration ou on le charge 
                redirect("romanobanderas/adminPane");

            }else{
  
                // L'utilisateur n'existe pas dans le système
                $error_data['login_fail'] ='fail';
                $this->load->view('template/header');
                $this->load->view('adminsyst/signin',$error_data);
                $this->load->view('template/footer'); 
